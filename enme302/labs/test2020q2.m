...
I1 = 6*10^-6;            % frame area moment, m^4
I2 = I1

Is = [I1, I2];

alpha1 = 80;         % angle of frame 1
...
%=========================================================================%
% ASSEMBLY MATRICES
A1 = [zeros(3) eye(3); zeros(1,6)];

A2 = [eye(3) zeros(3); 0 0 0 0 1 0];
...
if printStressStrain
    stresses = zeros(1,n_elements);
    fprintf("\n")
    fprintf("%s\n", spacerString)
    sum = 0;
    epss = zeros(1, n_elements);
    for i = 1:n_elements
        f = fs(:,:,i);
        ...
        L = Ls(i);
        epss(i) = (d(4) - d(1))/L;
        fprintf("strain in member %d: %.4f microstrain\n", i, epss(i)*1e6)
