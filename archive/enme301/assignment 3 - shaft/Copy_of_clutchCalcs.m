...
D_d_array = (50:0.5:500)/1000; % starting at 50mm because anything below that makes actuation force crazy
F_array_Tw = zeros(5, length(D_d_array));
F_array_Tp = zeros(5, length(D_d_array));
i = 1;
while i <= length(D_d_array)
D = D_d_array(i); % let D = maximum allowable 500mm
d = 0.5 * D; % let d = 0.5D
